...
module.exports = class AdvancedMissingFatArrows

	rule:
		name: 'advanced_missing_fat_arrows'
		level: 'ignore'
		is_strict: false
		...
		description: '''
			...
			Can be disabled with a first-line comment: ### coffeelint:bound_function ###
			'''
	...
	hasUseThisDirective: (node) ->
		node.body?.expressions?[0]?.base?.value in ['"use this"', "'use this'"]
	needsFatArrow: (node) ->
		@isCode(node) and (
			any(node.params, (param) => param.contains(@isThis)?) or
			containsButIsnt(node.body, @isThis, @isClass)
		) and not @hasUseThisDirective(node)
